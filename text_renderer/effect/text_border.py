...
class TextBorder(Effect):
    ...
    def apply(self, img: PILImage, text_bbox: BBox) -> Tuple[PILImage, BBox]:
        ...
        if self.border_style == 'solid':
            ...
            result_img = self._apply_dashed_border(result_img, border_mask, border_color, border_width)
        elif self.border_style == 'dotted':
            result_img = self._apply_dotted_border(result_img, border_mask, border_color, border_width)
    ...
    def _get_border_color(self, img: PILImage, text_bbox: BBox) -> Tuple[int, int, int, int]:
    ...
    def _create_light_border_color(self, text_color: Tuple[int, int, int]) -> Tuple[int, int, int, int]:
    ...
    def _create_dark_border_color(self, text_color: Tuple[int, int, int]) -> Tuple[int, int, int, int]:
    ...
    def _apply_solid_border(self, img: PILImage, border_mask: PILImage, border_color: Tuple[int, int, int, int]) -> PILImage:
        """Apply solid border"""
        # Create border image
    ...
    def _apply_dashed_border(self, img: PILImage, border_mask: PILImage, border_color: Tuple[int, int, int, int], border_width: int) -> PILImage:
        """Apply dashed border"""
    ...
    def _apply_dotted_border(self, img: PILImage, border_mask: PILImage, border_color: Tuple[int, int, int, int], border_width: int) -> PILImage:
        """Apply dotted border"""
            ...
            for j in range(0, border_array.shape[1], dot_spacing):
                if i < border_array.shape[0] and j < border_array.shape[1]:
                    ...
                    if border_array[i, j] > 0:  # Only add dots where border exists
                        dotted_mask[start_i:end_i, start_j:end_j] = border_array[start_i:end_i, start_j:end_j]
